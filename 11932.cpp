...
void dfs(int u);
void dfs2(int u);
void su(int i1, int i2, lli v, lli s = 0, lli e = MAX);
lli sq(int u, int v, int lca, int plca, int k, lli s = 0, lli e = MAX);
lli q(int u, int v, int k);
...
int main()
{
	...
	{
		int u = rev[i];
		su(num[p[u]], i, arr[u] - INT_MIN);
	}
	while (M--)
	{
		int x, y, k;
		cin >> x >> y >> k;
		cout << q(x, y, k) + INT_MIN << "\n";
	}
...
void su(int i1, int i2, lli v, lli s, lli e)
{
	if (s > v || e < v) return;
	pst[i2].v = pst[i1].v + 1;
	if (s == e) return;
...
lli sq(int u, int v, int lca, int plca, int k, lli s, lli e)
{
	if (s == e) return s;
	lli m = s + e >> 1;
