...
int N, M, K, f[MAX][MAX], c[MAX][MAX], lv[MAX], w[MAX], src = 0, dst = 2001;
vector<int> adj[MAX];
queue<int> q;
...
int main()
{
	...
	for (int i = 1; i <= N; ++i)
	{
		...
		c[2002][i] = 1;
		adj[2002].emplace_back(i);
		adj[i].emplace_back(2002);
	}
	c[src][2002] = K;
	adj[src].emplace_back(2002);
	adj[2002].emplace_back(src);
	for (int i = 1; i <= M; ++i)
	{
		c[bias + i][dst] = 1;
