...
constexpr int sz = 400, src = 0, snk = 367, INF = 0x3f3f3f3f;
...
int main()
{
	...
	while (cin >> N)
	{
		...
		for (int i = 1; i <= N; ++i)
		{
			...
			cin >> l >> r >> c;
			r++;
			adj[l].emplace_back(r, adj[r].size(), 1, -c);
			adj[r].emplace_back(l, adj[l].size() - 1, 0, c);
		}
		for (int i = 0; i <= 366; ++i)
		{
			adj[i].emplace_back(i + 1, adj[i + 1].size(), 2, 0);
